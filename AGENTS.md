...
- Tools (`src/fitness/fitness-tools.service.ts`, executors behind `FITNESS_REPO` — parameterized queries only, model never writes SQL):
  - `log_workout` `{date?, exercises: [{exercise, sets: [{reps, weight_kg?}]}], notes?}` — SHORT clean sessions only (≤ ~4 exercises).
  - `log_workout_text` `{raw_text, date?}` — long/messy real gym logs: the chat model passes the user's text VERBATIM (tiny tool call, no giant JSON mid-chat). `ExtractionService` (`src/fitness/extraction.service.ts`) then runs a dedicated ollama call: `format:` = strict JSON Schema (structured outputs — constrained decoding guarantees shape), `think: true` (parse quality > latency), model `EXTRACT_MODEL` (default `LLM_MODEL`), own budget `EXTRACT_TIMEOUT_MS` (default 420s). The gym-log disambiguation rules live in `EXTRACTION_SYSTEM_PROMPT` (AxB = A sets of B reps; ambiguous "12x3" → the 2-6-range number is the set count; European comma decimals; "28kg, 12,10,9" = 3 descending-rep sets; number-only lines continue the PREVIOUS exercise, never an exercise named "73kg"; machine settings/form remarks/questions → notes, never sets; names kept verbatim, lowercased). Schema uses set GROUPS ({sets, reps, weight_kg} — "3x8@93" = one group) expanded server-side: qwen3 collapses repeated identical set objects, so never make it repeat them. `options.num_ctx` 8192 (`EXTRACT_NUM_CTX`) — thinking + long log silently truncates at ollama's 4096 default. Result validated (group sets 1-10, reps 1-100, weight ≤500 or null, drops → `skipped[]`) then persisted via `repo.createWorkout`; summary `{logged, exercises, sets, skipped, notes}` goes back to the chat model.
